...
{   
    // Hard to parallelize - so I just combined all of them in a single loop to improve
    // time complexity..
    for (int j = 1; j < static_cast<int>(lbmData.dimY) - 1; j++) {
        const int j_rev = (static_cast<int>(lbmData.dimY) - 1) - j;
        const int row = j * lbmData.dimX;
        const int rown = (j + 1) * lbmData.dimX;
        const int row_rev = j_rev * lbmData.dimX;
        const int rowp = (j_rev - 1) * lbmData.dimX;
    
        // Process SW (current j) and NW (j_rev) directions with i forward
        for (int i = 1; i < static_cast<int>(lbmData.dimX) - 1; i++) {
            // SW streaming
            lbmData.fW[row + i]  = lbmData.fW[row + i + 1];
            lbmData.fSW[row + i] = lbmData.fSW[rown + i + 1];
            // NW streaming
            lbmData.fN[row_rev + i]  = lbmData.fN[rowp + i];
            lbmData.fNW[row_rev + i] = lbmData.fNW[rowp + i + 1];
        }
    
        // Process SE (current j) and NE (j_rev) directions with i backward
        for (int i = static_cast<int>(lbmData.dimX) - 2; i > 0; i--) {
            // SE streaming
            lbmData.fS[row + i]  = lbmData.fS[rown + i];
            lbmData.fSE[row + i] = lbmData.fSE[rown + i - 1];
            // NE streaming
            lbmData.fE[row_rev + i]  = lbmData.fE[row_rev + i - 1];
            lbmData.fNE[row_rev + i] = lbmData.fNE[rowp + i - 1];
        }
    }
}
